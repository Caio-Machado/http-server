...
impl Request {
    ...
    pub fn extract_request_fields(request_string: String) -> Result<(StartLine, Vec<Header>, Option<String>), RequestError> {
        ...
        let header: Vec<Header> = match Header::try_build_headers(splited_request.get(1..splited_request.len() -1)) {
            Ok(headers) => headers,
            Err(e) => return Err(RequestError::InvalidHeader),
        };
        let body: Option<String> = match splited_request.last() {
            Some(body_str) => {
                match body_str.to_owned() {
                    "" => None,
    ...
    pub fn build(start_line_string: Option<&&str>) -> Result<StartLine, RequestError> {
        match start_line_string {
            Some(stl_string) => {
                ...
                let http_version: HttpVersion = match HttpVersion::identify_http_version(fields.get(2)) {
                    Ok(http_v) => http_v,
                    Err(e) => return Err(e),
                };
...
impl RequestTarget {
    pub fn build(possible_full_path: Option<&&str>) -> Result<RequestTarget, RequestError> {
        match possible_full_path {
            Some(full_path) => {
                let splited_path: Vec<&str> = full_path.splitn(3, "/").filter(|s| s != &"").collect();
...
impl HttpVersion {
    fn  identify_http_version(possible_http_version: Option<&&str>) -> Result<HttpVersion, RequestError> {
        match possible_http_version {
            Some(http_version) => {
                match http_version.to_owned() {
